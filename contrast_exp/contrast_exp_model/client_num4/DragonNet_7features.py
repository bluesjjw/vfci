...
# phase = ["train", "eval"]
class Dragonnet_7features(nn.Module):
    def __init__(self):
        super(Dragonnet_7features, self).__init__()
        self.rep_init = nn.Linear(in_features=7, out_features=7)
        nn.init.xavier_uniform_(self.rep_init.weight)

        # representation
        self.rep_fc1 = nn.Linear(in_features=7, out_features=56)
        nn.init.xavier_uniform_(self.rep_fc1.weight)

        self.rep_fc2 = nn.Linear(in_features=56, out_features=56)
        nn.init.xavier_uniform_(self.rep_fc2.weight)

        self.rep_fc3 = nn.Linear(in_features=56, out_features=56)
        nn.init.xavier_uniform_(self.rep_fc3.weight)

        self.t_predictions = nn.Linear(in_features=56, out_features=1)
        nn.init.xavier_uniform_(self.t_predictions.weight)

        # HYPOTHESIS
        self.y0_hidden_fc1 = nn.Linear(in_features=56, out_features=28)
        nn.init.xavier_uniform_(self.y0_hidden_fc1.weight)
        self.y1_hidden_fc1 = nn.Linear(in_features=56, out_features=28)
        nn.init.xavier_uniform_(self.y1_hidden_fc1.weight)
        self.y0_hidden_fc2 = nn.Linear(in_features=28, out_features=28)
        nn.init.xavier_uniform_(self.y0_hidden_fc2.weight)
        self.y1_hidden_fc2 = nn.Linear(in_features=28, out_features=28)
        nn.init.xavier_uniform_(self.y1_hidden_fc2.weight)

        self.y0_predictions = nn.Linear(in_features=28, out_features=1)
        nn.init.xavier_uniform_(self.y0_predictions.weight)
        self.y1_predictions = nn.Linear(in_features=28, out_features=1)
