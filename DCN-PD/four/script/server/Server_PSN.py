...
class DataTransServer(tenseal_psndata_pb2_grpc.SafeTransmissionServicer):
    ...
    def getFeatures(self):
        ...
        rows = int(flatten_features.size(0) / 7)
        features = flatten_features.unflatten(0, (rows, 7))
        # print(features)
        for client in range(1, self.num_clients):
            features_list = self.vectors_list[client]
            flatten_features = torch.tensor(features_list)
            rows = int(flatten_features.size(0) / 6)
            unflatten_features = flatten_features.unflatten(0, (rows, 6))
...
def serve():
    ...
    tenseal_psndata_pb2_grpc.add_SafeTransmissionServicer_to_server(DataTransServer(1, 4, ctx_file), server)
