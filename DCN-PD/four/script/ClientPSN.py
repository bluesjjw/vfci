...
# import tenseal_data_pb2
# import tenseal_data_pb2_grpc
import grpc
...
def run(rank):
    ...
    dataset = torch.utils.data.TensorDataset(idx_train, ps_train_x)
    ...
    # checkpint=torch.load('../checkpoints/Bottom/2client_all_features_epoch_50_Bottom{0}_checkpoint.pth'.format(rank))
    ...
    for epo in tqdm(range(1, 51)):
        ...
        for batch in data_loader:
            ...
            if rank==0:
                rows = int(schegrad.size(0) / 7)
                schegrad = schegrad.unflatten(0, (rows, 7))
            else:
                rows = int(schegrad.size(0) / 6)
                schegrad = schegrad.unflatten(0, (rows, 6))
