...
sys.path.append("..")
from model.MergeDCN_12features import MergeDCN_12features
from model.MergeDCN_13features import MergeDCN_13features
...
def trainDCN(dcn_net,treated_dataset, control_dataset):
    ...
    torch.save(network.state_dict(), "../save/4client_select/select_two/MergeDCN_{0}_{1}.pth".format(client_1,client_2))
...
if __name__ == "__main__":
    ...
    psn_model_path = "../save/4client_select/select_two/MergePSN_{0}_{1}.pth".format(client_1,client_2)
    ...
    dcn_network = MergeDCN_12features(training_flag=True).cuda()
    psn_network = network =MergePSN_12features("eval").cuda()
    if client_1 ==1 or client_2==1:
        psn_network =MergePSN_13features("eval").cuda()
        dcn_network = MergeDCN_13features(training_flag=True).cuda()
