...
class ScheDCN(nn.Module):
    def __init__(self, training_flag):
        super(ScheDCN, self).__init__()
        self.training = training_flag

        # shared layer
        self.shared1 = nn.Linear(in_features=5, out_features=40)
        nn.init.xavier_uniform_(self.shared1.weight)

        self.shared2 = nn.Linear(in_features=40, out_features=40)
