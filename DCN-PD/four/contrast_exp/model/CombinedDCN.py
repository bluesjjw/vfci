...
class CombinedDCN(nn.Module):
    def __init__(self, training_flag):
        super(CombinedDCN, self).__init__()
        self.training = training_flag

        # shared layer
        self.shared1 = nn.Linear(in_features=24, out_features=192)
        nn.init.xavier_uniform_(self.shared1.weight)

        self.shared2 = nn.Linear(in_features=192, out_features=192)
        nn.init.xavier_uniform_(self.shared2.weight)

        # potential outcome1 Y(1)
        self.hidden1_Y1 = nn.Linear(in_features=192, out_features=192)
        nn.init.xavier_uniform_(self.hidden1_Y1.weight)

        self.hidden2_Y1 = nn.Linear(in_features=192, out_features=192)
        nn.init.xavier_uniform_(self.hidden2_Y1.weight)

        self.out_Y1 = nn.Linear(in_features=192, out_features=1)
        nn.init.xavier_uniform_(self.out_Y1.weight)

        # potential outcome1 Y(0)
        self.hidden1_Y0 = nn.Linear(in_features=192, out_features=192)
        nn.init.xavier_uniform_(self.hidden1_Y0.weight)

        self.hidden2_Y0 = nn.Linear(in_features=192, out_features=192)
        nn.init.xavier_uniform_(self.hidden2_Y0.weight)

        self.out_Y0 = nn.Linear(in_features=192, out_features=1)
