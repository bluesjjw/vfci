...
class DataPartition:
    ...
    def getDCNTensorWithLabel(self, csv_path, phase):
        ...
        # treated data load
        treat_np_ID, treat_covariates_X, treat_treatment_Y, treat_outcome_Y = \
        ...
        treat_out_y_train, treat_out_y_test = train_test_split(treat_np_ID, treat_covariates_X, treat_treatment_Y,
                                                               treat_outcome_Y, random_state=self.seed,
                                                               train_size=0.8)
        ...
        # control data load
        control_np_ID, control_covariates_X, control_treatment_Y, control_outcome_Y = \
        ...
        control_out_y_train, control_out_y_test = train_test_split(control_np_ID, control_covariates_X,
                                                                   control_treatment_Y, control_outcome_Y,
                                                                   random_state=self.seed, train_size=0.8)
    ...
    def getIndex(self, csv_path):
        df = pd.read_csv(csv_path, header=None)
        treat_df = df[df.iloc[:, -3] == 1]
        control_df = df[df.iloc[:, -3] == 0]
