...
from models.client_num4.MiddleModel import MiddleModel
...
class DataTransServer(tenseal_DragonNetData_pb2_grpc.SafeTransmissionServicer):
    ...
    def getFeatures(self):
        ...
        rows = int(flatten_features.size(0) / 56)
        features = flatten_features.unflatten(0, (rows, 56))
        # print(features)
        for client in range(1, self.num_clients):
            features_list = self.vectors_list[client]
            flatten_features = torch.tensor(features_list)
            rows = int(flatten_features.size(0) / 48)
            unflatten_features = flatten_features.unflatten(0, (rows, 48))
            features = torch.cat((features, unflatten_features), dim=-1)
        # print(features)
        return features

    def DragonNetForward(self, request, context):
        ...
        if client_rank == self.num_clients - 1:
            ...
            self.concat_pred = forwardMiddle(self.DragonNet, self.features)
            # print(self.concat_pred)
            self.forward_completed = True

        while not self.forward_completed:
            time.sleep(self.sleep_time)

        plain_vector = ts.plain_tensor(self.concat_pred.detach().cpu().flatten())
    ...
    def DragonNetBackward(self, request, context):
        ...
        outgrad = backwardMiddle(ingrad, self.optimizer, self.concat_pred, self.features)
        num_list = [56, 48, 48, 48]
    ...
    def DistDenseBackward(self, request, context):
        client_rank = request.client_rank
        rnd = request.round
...
def serve():
    ...
    tenseal_DragonNetData_pb2_grpc.add_SafeTransmissionServicer_to_server(DataTransServer(1, 4, ctx_file), server)
