...
sys.path.append('..')
from models.client_num4.Bottom_7features import Bottom_7features
from models.client_num4.Bottom_6features import Bottom_6features
...
def run(rank):
    ...
    network = Bottom_6features().cuda()
    if rank == 0:
        network = Bottom_7features().cuda()
    model_save_path = ""
    check_save_path = ""
    ...
    for epo in tqdm(range(1, 401)):
        ...
        for batch in data_loader:
            ...
            rows = int(schegrad.size(0) / 48)
            schegrad = schegrad.unflatten(0, (rows, 48))

            if rank == 0:
                rows = int(schegrad.size(0) / 56)
                schegrad = schegrad.unflatten(0, (rows, 56))
